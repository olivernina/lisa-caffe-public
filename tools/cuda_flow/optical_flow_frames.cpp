...
int main( int argc, char *argv[] )
{
    ...
    if( argc != 3 )
    {
        ...
        cout << "The format is: ./in.avi outdir/in/*.jpg" << endl;
        return -1;
    }
...
//    namedWindow("Motion Flow",CV_WINDOW_NORMAL);
    ...
    //	outputVideo.open(argv[2], static_cast<int>(cap.get(CV_CAP_PROP_FOURCC)),cap.get(CV_CAP_PROP_FPS),S,true);
...
//            waitKey(3);
        ...
        {
	  //	  sprintf(cad,"./output/motion/image%04d.jpg",argv[2],nframes);
	  //imwrite(cad,motion_flow);

	  sprintf(cad,"%s/image%04d.jpg",argv[2],nframes);
            imwrite(cad,flow_rgb);

	    //            outputVideo<<flow_rgb;
        }
