...
def main(argv):
  ...
  video_path_in= args.video
  RGB_video_path_out = 'frames/'
  flow_video_path_out = 'flow_images/'

  if not os.path.exists(RGB_video_path_out):
     os.mkdir(RGB_video_path_out)

  if not os.path.exists(flow_video_path_out):
     os.mkdir(flow_video_path_out)
  ...
  video_frames = os.path.join(RGB_video_path_out, video_dir)
  if not os.path.exists(video_frames):
    os.mkdir(video_frames)
    os.system('ffmpeg -i '+video_path_in+' -an -vf scale="256:256"  '+video_frames+'/%4d.jpg')

  flow_frames = os.path.join(flow_video_path_out, video_dir)
  if not os.path.exists(flow_frames):
     os.mkdir(flow_frames)
     os.system('./optical_flow_frames '+video_path_in+' '+flow_frames)


  # Extract list of frames in video
  RGB_frames = glob.glob(video_frames+'/*.jpg')
  flow_frames = glob.glob(flow_frames+'/*.jpg')
  ...
  final_video_path_out = 'output/' #TODO: this could be passed as an argument
  if not os.path.exists(final_video_path_out):
     os.mkdir(final_video_path_out)

  video_frames_out = os.path.join(final_video_path_out, video_dir)
  if not os.path.exists(video_frames_out):
    os.mkdir(video_frames_out)
    os.system('ffmpeg -i '+video_path_in+' -an '+video_frames_out+'/%4d.jpg')

  RGB_frames_out = glob.glob(video_frames_out+'/*.jpg')


  label = action_hash[compute_fusion(predictions_RGB_LRCN, predictions_flow_LRCN, 0.5)]
  gt_label = video_path_in.split('_')[1]

  from PIL import Image
  from PIL import ImageDraw
  from PIL import ImageFont
  for image in RGB_frames_out:
      img = Image.open(image)
      draw = ImageDraw.Draw(img)
      draw.rectangle((0,0, len(label)*11,16*1.5), fill=(0,0,0))
      font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSans-Bold.ttf", 16)
      color = (0,255,0) if (gt_label == label) else (255,0,0)
      draw.text((5, 5),label,color,font=font)
      img.save(image)
  ...
  os.system('ffmpeg -framerate 30 -i '+video_frames_out+'/%4d.jpg'+' -r 30 '+os.path.join(final_video_path_out,(video_dir+'.mp4')))

  os.system('convert -bordercolor white -border 0 -layers OptimizePlus -delay 3x100 '+video_frames_out+'/*.jpg'+' -loop 0 '+os.path.join(final_video_path_out,(video_dir+'.gif')))
