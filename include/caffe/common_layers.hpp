...
/**
 * @brief An InnerProductLayer with num_outputs = num_inputs which may be non-zero
 *        only along the diagonal.
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
class DiagInnerProductLayer : public Layer<Dtype> {
 public:
  explicit DiagInnerProductLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_DIAG_INNER_PRODUCT;
  }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
 ...
 protected:
  ...
  int dim_;
  Blob<Dtype> weight_diff_buffer_;
};
...
/**
 * @brief Convert an (n, 1, h, w) Blob of indices [0, 1, ..., c - 1]
 *        to a one-hot encoded (n, c, h, w) Blob.
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
class Idx2OneHotLayer : public Layer<Dtype> {
 public:
  explicit Idx2OneHotLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_IDX21HOT;
  }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
...
class ReductionLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_REDUCTION;
  }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
