...
class ResetRNNLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ResetRNNLayerTest() : hidden_dim_(3) {
...
TYPED_TEST_CASE(ResetRNNLayerTest, TestDtypesAndDevices);

TYPED_TEST(ResetRNNLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  ResetRNNLayer<Dtype> layer(this->layer_param_);
...
TYPED_TEST(ResetRNNLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<ResetRNNLayer<Dtype> > layer(
      new ResetRNNLayer<Dtype>(this->layer_param_));
  ...
  Caffe::set_random_seed(1701);
  layer.reset(new ResetRNNLayer<Dtype>(this->layer_param_));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(ResetRNNLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  ResetRNNLayer<Dtype> layer(this->layer_param_);
...
TYPED_TEST(ResetRNNLayerTest, TestGradientNonZeroFlush) {
...
TYPED_TEST(ResetRNNLayerTest, TestGradientNonZeroFlushBufferSize2) {
  Caffe::set_phase(Caffe::TEST);
  typedef typename TypeParam::Dtype Dtype;
