...
template <typename Dtype>
void LRNLayer<Dtype>::PartialChannelForward_cpu(
  ...
  int channels_per_division = channels_/num_divisor;
  for (int i = 0; i < scale_.count(); ++i) {
    scale_data[i] = k_;
  }
  for (int nd = 0; nd < num_divisor; ++nd){
    int c_start = nd*channels_per_division;
    int c_end = (nd+1)*channels_per_division;
    //for (int i = c_start*height*width; i < scale_.count(); ++i) {
    Blob<Dtype> padded_square(1, channels_per_division + size_ - 1, height_, width_);
    ...
    for (int n = 0; n < num_; ++n) {
      ...
      //num elements, input, output
             //NOT SURE WHAT THIS IS HERE FOR???
      caffe_sqr(channels_per_division * height_ * width_,
          bottom_data + bottom[0]->offset(n),
          padded_square_data + padded_square.offset(0, pre_pad_));
      // Create the first channel scale
      for (int c = c_start; c < c_start + size_; ++c) {
        caffe_axpy<Dtype>(height_ * width_, alpha_over_size,
            padded_square_data + padded_square.offset(0, c-c_start),
            scale_data + scale_.offset(n, c_start));
      }
      for (int c = c_start+1; c < c_end; ++c) {
        ...
        // add head
        caffe_axpy<Dtype>(height_ * width_, alpha_over_size,
            padded_square_data + padded_square.offset(0, c - c_start + size_ - 1),
            scale_data + scale_.offset(n, c));
        // subtract tail
        caffe_axpy<Dtype>(height_ * width_, -alpha_over_size,
            padded_square_data + padded_square.offset(0, c -c_start - 1),
            scale_data + scale_.offset(n, c));
      }
    }
  }
  // In the end, compute output
  //num elements, input vector, constant power, output vector.
  caffe_powx<Dtype>(scale_.count(), scale_data, -beta_, top_data);
  //numper of elements, input vector1, input vector2, output vector.
...
template <typename Dtype>
void LRNLayer<Dtype>::PartialChannelBackward_cpu(
    const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  const Dtype* top_diff = top[0]->cpu_diff();
  const Dtype* top_data = top[0]->cpu_data();
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    // Now, compute the accumulated ratios and the bottom diff
    caffe_set(accum_ratio.count(), Dtype(0), accum_ratio_data);
    for (int cc = 0; cc < num_divisor; ++cc){
      for (int c = cc*channels_per_division; c < size_ - 1; ++c) {
        caffe_axpy<Dtype>(height_ * width_, 1.,
            padded_ratio_data + padded_ratio.offset(0, c), accum_ratio_data);
      }
    }
    for (int cc = 0; cc < num_divisor; ++cc){
      for (int c = cc*channels_per_division; c < (cc+1)*channels_per_division; ++c) {
        caffe_axpy<Dtype>(height_ * width_, 1.,
            padded_ratio_data + padded_ratio.offset(0, c + size_ - 1),
            accum_ratio_data);
        // compute bottom diff
